...
describe("deleteAllForecastRuns", () => {
  ...
  it("cancels a running benchmark before deleting runs", async () => {
    ...
    const runsListIndex = fetchMock.mock.calls.findIndex(
      ([url, init]) =>
        (init?.method ?? "GET") === "GET" &&
        String(url).includes("/forecaster/runs"),
    );
    const lastStatusBeforeRuns = fetchMock.mock.calls
      .slice(0, runsListIndex)
      .filter(
        ([url, init]) =>
          (init?.method ?? "GET") === "GET" &&
          String(url).endsWith("/forecaster"),
      )
      .at(-1);
    expect(lastStatusBeforeRuns).toBeDefined();
  });
