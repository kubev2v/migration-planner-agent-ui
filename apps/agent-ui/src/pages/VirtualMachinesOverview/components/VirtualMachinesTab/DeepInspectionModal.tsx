...
export const DeepInspectionModal: React.FC<DeepInspectionModalProps> = ({
  ...
  useEffect(() => {
    knownVmsForInspectionRef.current = knownVmsForInspection;
  }, [knownVmsForInspection]);
      ...
      try {
        ...
        if (running) {
          const vmIdsUnderInspection = await fetchVmIdsUnderInspection(
            agentApi,
            knownVmsForInspectionRef.current,
          );
          if (cancelled) {
            return;
          }
  ...
  }, [agentApi, fetchExistingStatus, getInspectorRunningState, isOpen]);
  ...
  const canConfigure =
    ...
    (!hasVMsSelected ||
      (!loadingInspectionContext && !inspectionContextError && !tooManyVMs));
    ...
    >
      ...
      <ModalFooter>
        <Button
          ...
          isLoading={
            configuring || (hasVMsSelected && loadingInspectionContext)
          }
          isDisabled={
            !canConfigure ||
            configuring ||
            (hasVMsSelected && loadingInspectionContext)
          }
        >
          {hasVMsSelected
            ? addingToExistingInspection
