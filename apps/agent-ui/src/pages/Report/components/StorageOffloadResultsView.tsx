...
> = ({
  ...
  const drawerRuns = drawerPair
    ? (runsByPairName.get(drawerPair.name) ?? [])
    : [];
    ...
    <DrawerPanelContent>
      ...
      <DrawerPanelBody>
        ...
        <Content component="p" style={{ marginBottom: "16px" }}>
          <strong>Target data store:</strong> {drawerPair.targetDatastore}
        </Content>
        {drawerRuns.length === 0 ? (
                ...
                <Tbody>
                  {filteredPairs.length === 0 ? (
                    ...
                    filteredPairs.map((pair) => {
                      ...
                      const pairRuns = runsByPairName.get(pair.name) ?? [];
